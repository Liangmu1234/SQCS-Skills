---
name: tdx-connector
description: "Query global stock data via Tongdaxin MCP, including quotes, screening, reports, notices, news, and macro research. On Windows PowerShell 5.1, always read this UTF-8 SKILL.md and related text files with Get-Content -Encoding UTF8 to prevent mojibake."
---
...
在 Windows PowerShell 5.1 中读取本 Skill 或其 Markdown、JSON、YAML 文本文件时，始终显式指定 UTF-8：

```powershell
Get-Content -Raw -Encoding UTF8 -LiteralPath $path
```

不要使用未指定 `-Encoding` 的 `Get-Content`；无 BOM 的 UTF-8 文件会被 Windows PowerShell 5.1 按系统 ANSI/GBK 解码并显示为乱码。
